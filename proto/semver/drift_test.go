...
func TestProtoHashDrift(t *testing.T) {
	protoBytes, err := os.ReadFile("proto/semver.proto")
	if err != nil {
		t.Fatalf("reading proto source: %v", err)
	}
	wantHash := fmt.Sprintf("%x", sha256.Sum256(protoBytes))

	gotHashBytes, err := os.ReadFile("proto/semver/.proto_hash")
	if err != nil {
	...
	if gotHash != wantHash {
		t.Errorf("proto/semver.proto has changed since .pb.go files were generated\n"+
			"  stored hash: %s\n"+
			"  current hash: %s",
			gotHash, wantHash)
	}
